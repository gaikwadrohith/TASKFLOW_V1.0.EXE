...
function Field({ label, value, onChange, type = "text", readOnly = false }) {
  return (
    <div className="mb-4">
      <p className="text-xs tracking-widest mb-1" style={{ color: "var(--gray)", fontFamily: "var(--font)" }}>
        {label}
      </p>
      <input
        type={type}
...
function Toggle({ label, description, value, onChange }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      <div>
        <p className="text-xs font-bold tracking-widest" style={{ fontFamily: "var(--font)", color: "var(--black)" }}>
          {label}
        </p>
        {description && (
      ...
      <div className="hidden xl:flex flex-col gap-4 w-44 flex-shrink-0 pt-10" style={{ fontFamily: "var(--font)" }}>
          ...
          <div className="p-3 flex flex-col gap-1.5">
            {[
              ...
              <div key={key} className="flex items-center justify-between">
                <span className="border border-black px-1.5 py-0.5 text-xs font-bold" style={{ background: "var(--pink-light)", fontSize: "0.55rem", color: "var(--black)" }}>
                  {key}
                </span>
        ...
        <div className="border-2 border-black p-3" style={{ background: "#f0fdf4" }}>
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-xs font-bold tracking-widest" style={{ color: "#16a34a", fontSize: "0.55rem" }}>
              SECURE
            </span>
          </div>
          <p className="text-xs" style={{ color: "#16a34a", fontSize: "0.5rem" }}>
            ACCOUNT PROTECTED
          </p>
          <p className="text-xs" style={{ color: "#16a34a", fontSize: "0.5rem" }}>
            DATA ENCRYPTED
      ...
      <div className="hidden xl:flex flex-col gap-4 w-44 flex-shrink-0 pt-10" style={{ fontFamily: "var(--font)" }}>
          ...
          <div className="p-3">
            {["[OK] AUTH_MODULE", "[OK] REDUX_STORE", "[OK] THEME_ENGINE", "[OK] TASK_SYNC", "[OK] DATA_PERSIST"].map((line, i) => (
              <Motion.p
                ...
                style={{ color: "#4ade80", fontFamily: "var(--font)", fontSize: "0.5rem" }}
              >
                {line}
              </Motion.p>
            ))}
        ...
        <div className="border-2 border-black overflow-hidden" style={{ background: "var(--white)" }}>
          ...
          <div className="p-4 text-center">
            <p className="text-3xl font-bold" style={{ color: "var(--black)", letterSpacing: "-0.04em" }}>
              {total}
            </p>
            <p className="text-xs tracking-widest" style={{ color: "var(--gray)", fontSize: "0.55rem" }}>
              / 100 TASKS
            </p>
        ...
        <div className="border-2 border-black p-3" style={{ background: "var(--white)" }}>
          <p className="text-xs font-bold tracking-widest mb-2" style={{ color: "var(--black)", fontSize: "0.55rem" }}>
            TASKFLOW_V1.0
          </p>
          {[
            ["REACT", "18"],
            ...
            <div key={k} className="flex justify-between py-0.5">
              ...
              <span className="text-xs font-bold" style={{ fontSize: "0.5rem", color: "var(--black)" }}>
                v{v}
              </span>
            </div>
          ))}
...
export default function Settings({ setAuth }) {
  ...
  const [activeTheme, setActiveTheme] = useState(() => {
    const saved = localStorage.getItem("theme") || "MINT";
    applyTheme(saved);
    return saved;
  });
      ...
      <div className="flex-1 flex gap-6 w-full max-w-7xl mx-auto px-6 pb-16">
        ...
        <div className="flex-1 mt-8 min-w-0">
          ...
          <Motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-3 gap-2 mb-4">
            {[
              ...
              <div key={s.label} className="border-2 border-black p-3 text-center" style={{ background: "var(--white)" }}>
                <p className="text-3xl font-bold" style={{ letterSpacing: "-0.04em", color: "var(--black)" }}>
                  {s.value}
                </p>
          ...
          <Section icon={<FaPalette />} title="APPEARANCE" delay={0.15}>
            <p className="text-xs tracking-widest mb-3" style={{ color: "var(--gray)" }}>
          ...
          <Section icon={<FaDownload />} title="DATA_MANAGEMENT" delay={0.35}>
            ...
            <div className="border-2 border-black p-3" style={{ background: "#fafafa" }}>
              <p className="text-xs tracking-widest mb-2" style={{ color: "var(--gray)" }}>
                STORAGE_USAGE
              </p>
          ...
          <Section icon={<FaShieldAlt />} title="DANGER_ZONE" delay={0.45}>
            <div className="flex items-center justify-between py-3 border-b border-gray-100">
              <div>
                <p className="text-xs font-bold tracking-widest" style={{ color: "var(--black)" }}>
                  CLEAR_COMPLETED
                </p>
