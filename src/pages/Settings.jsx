...
function Field({ label, value, onChange, type = "text", readOnly = false }) {
  return (
    <div className="mb-4">
      <p className="text-xs tracking-widest text-gray-400 mb-1" style={{ fontFamily: "var(--font)" }}>
        {label}
      </p>
      <input
        type={type}
        ...
        style={{
          fontFamily: "var(--font)",
          background: readOnly ? "#f5f5f5" : "var(--white)",
          color: "var(--black)",
        }}
      />
    </div>
...
function Toggle({ label, description, value, onChange }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      <div>
        <p className="text-xs font-bold tracking-widest" style={{ fontFamily: "var(--font)" }}>
          {label}
        </p>
        {description && (
          <p className="text-xs text-gray-400 mt-0.5" style={{ fontFamily: "var(--font)" }}>
            {description}
          </p>
        )}
      </div>
      <button onClick={() => onChange(!value)} className="flex-shrink-0 w-12 h-6 border-2 border-black relative transition-all ml-4" style={{ background: value ? "var(--black)" : "transparent" }}>
        <Motion.span
...
export default function Settings({ setAuth }) {
  ...
  // ✅ Single fix — lazy initializer runs once, no useEffect needed
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        alert("PARSE_ERROR — INVALID JSON");
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "var(--font)" }}>
      ...
      <div className="flex-1 w-full max-w-2xl mx-auto px-5 mt-8 pb-16">
        {/* Page Title */}
        <Motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <p className="text-xs tracking-widest mb-1" style={{ color: "var(--gray)" }}>
            // TASKFLOW / SETTINGS
          </p>
          <h1 className="text-5xl font-bold" style={{ fontStyle: "italic", letterSpacing: "-0.03em" }}>
            SETTINGS
          </h1>
        ...
        {/* ── Account Stats ── */}
        <Motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            <div key={s.label} className="border-2 border-black p-3 text-center" style={{ background: "var(--white)" }}>
              <p className="text-3xl font-bold" style={{ letterSpacing: "-0.04em" }}>
                {s.value}
              </p>
              <p className="text-xs tracking-widest mt-1" style={{ color: "var(--gray)" }}>
                {s.label}
              </p>
            </div>
          ))}
        ...
        <Section icon={<FaPalette />} title="APPEARANCE" delay={0.15}>
          <p className="text-xs tracking-widest mb-3" style={{ color: "var(--gray)", fontFamily: "var(--font)" }}>
            SELECT_THEME — APPLIED INSTANTLY
          </p>
          <div className="grid grid-cols-5 gap-2 mb-2">
            {Object.entries(THEMES).map(([key, theme]) => (
              <button
                ...
                style={{
                  borderColor: activeTheme === key ? "var(--black)" : "#ccc",
                  background: activeTheme === key ? "var(--pink)" : "transparent",
                }}
              >
                {/* Color swatches */}
                ...
                <span className="text-xs font-bold tracking-widest" style={{ fontFamily: "var(--font)", fontSize: "0.55rem" }}>
                  {theme.label}
                </span>
                {activeTheme === key && <Motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--black)" }} />}
              </button>
            ))}
          </div>
          <p className="text-xs tracking-widest mt-2" style={{ color: "var(--gray)" }}>
            ACTIVE: {activeTheme}_THEME
          </p>
        </Section>
        ...
        <Section icon={<FaLock />} title="SECURITY" delay={0.3}>
          <div className="flex items-center gap-3 border-2 border-black px-4 py-3 mb-3" style={{ background: "#f0fdf4" }}>
            ...
            <span className="text-xs text-green-700 font-bold tracking-widest" style={{ fontFamily: "var(--font)" }}>
              ACCOUNT_SECURED
            </span>
          ...
          <p className="text-xs tracking-widest" style={{ color: "var(--gray)", fontFamily: "var(--font)" }}>
            TO CHANGE PASSWORD, UPDATE DIRECTLY IN DB.JSON
          </p>
        </Section>

        {/* ── Data Management ── */}
        <Section icon={<FaDownload />} title="DATA_MANAGEMENT" delay={0.35}>
          <p className="text-xs tracking-widest mb-4" style={{ color: "var(--gray)", fontFamily: "var(--font)" }}>
            EXPORT OR IMPORT YOUR TASK DATA AS JSON
          </p>
          <div className="flex gap-2 mb-3">
            ...
            {/* Import */}
            <label
              className="flex-1 flex items-center justify-center gap-2 border-2 border-black px-4 py-2.5 text-xs font-bold tracking-widest hover:bg-black hover:text-white transition cursor-pointer"
          ...
          <div className="border-2 border-black p-3" style={{ background: "#fafafa" }}>
            <p className="text-xs tracking-widest mb-2" style={{ color: "var(--gray)", fontFamily: "var(--font)" }}>
              STORAGE_USAGE
            </p>
        ...
        <Section icon={<FaInfo />} title="ABOUT" delay={0.4}>
          {[
            ...
            <div key={item.label} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
              ...
              <span className="text-xs font-bold tracking-widest" style={{ fontFamily: "var(--font)" }}>
                {item.value}
              </span>
            </div>
          ))}
        ...
        <Section icon={<FaShieldAlt />} title="DANGER_ZONE" delay={0.45}>
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
            <div>
              ...
              <p className="text-xs text-gray-400" style={{ fontFamily: "var(--font)" }}>
            ...
            <button
              ...
              style={{
                fontFamily: "var(--font)",
                borderColor: confirmClear ? "#dc2626" : "var(--black)",
                color: confirmClear ? "#dc2626" : "var(--black)",
          ...
          <div className="flex items-center justify-between pt-3">
            <div>
              <p className="text-xs font-bold tracking-widest text-red-500" style={{ fontFamily: "var(--font)" }}>
                DELETE_ACCOUNT
              </p>
              <p className="text-xs text-gray-400" style={{ fontFamily: "var(--font)" }}>
      ...
      {/* Footer */}
      <footer className="w-full border-t-2 border-black px-5 py-2 flex justify-between" style={{ background: "var(--black)", fontFamily: "var(--font)" }}>
